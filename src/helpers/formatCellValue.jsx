export default function formatCellValue(value, formatter, record = {}, columnInfo = {}) {
  ...
  switch (formatter?.toLowerCase()) {
      ...
      if (cleanValue.length > 40) {
        ...
        return (
          <div className="cursor-pointer">
            {abstract}
            <div className="hidden" dangerouslySetInnerHTML={{ __html: cleanValue }} />
          </div>
        );
      }
    ...
    case 'template':
      if (columnInfo?.template) {
        ...
        return columnInfo.template.replace(/%([\w.-]+)%/g, (_, key) => record[key] ?? '');
      }
      return String(value);
