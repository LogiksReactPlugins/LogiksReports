...
const GalleryView = ({
  ...
  const getCardColor = (row) => {
    ...
    const colorKey = getRowValue(row, cards.colmap.color);
            ...
            paginatedGroupedData[groupKey].length > 0 ? (
              paginatedGroupedData[groupKey].map((row, rowIndex) => (
                <div
                  ...
                  className={
                    style?.card ||
                    `border relative rounded-lg shadow-sm group hover:shadow-md transition-shadow duration-200 ${getCardColor(
                      row
                    )}`
                  }
                >
                  ...
                  <div className="px-2 pb-2">{/* Due Date */}</div>
